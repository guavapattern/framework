 ...
 *
 */
abstract class GPFramework_Options extends GPFramework_Abstract {
  ...
  public function element_class( $el_class = '' ) {
    ...
    return ( $field_class || $el_class ) ? ' class="'. $el_class . $field_class .'"' : 'class=""';
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="gp-text-desc">'. $this->field['info'] .'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'GP_OPTIONS_DEBUG' ) && GP_OPTIONS_DEBUG ) ) {

      $value = $this->element_value();

      $out .= "<pre>";
      $out .= "<strong>". __( 'CONFIG', 'gp-framework' ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". __( 'USAGE', 'gp-framework' ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "gp_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". __( 'VALUE', 'gp-framework' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
  ...
  public function element_get_error() {
    ...
    if( ! empty( $gp_errors ) ) {
      foreach ( $gp_errors as $key => $value ) {
        if( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
          $out .= '<p class="gp-text-warning">'. $value['message'] .'</p>';
        }
      }
    }
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="gp-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
  }
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      foreach ( $this->multilang['languages'] as $key => $val ) {
        ...
        if( $key != $this->multilang['current'] ) {
          ...
          $class = 'GPFramework_Option_' . $this->field['type'];
      ...
      $out .= '</fieldset>';
      $out .= '<p class="gp-text-desc">'. sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'gp-framework' ), $this->multilang['current'] ) .'</p>';
  ...
  public function element_multilang() {
    return ( isset( $this->field['multilang'] ) ) ? gp_language_defaults() : false;
  }

}

// load all of fields
gp_load_option_fields();
