 ...
 *
 */
class GPFramework_Shortcode_Manager extends GPFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'gp_shortcode_options', $options );
    $this->exclude_post_types = apply_filters( 'gp_shortcode_exclude', $this->exclude_post_types );
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'wp_ajax_gp-get-shortcode', 'shortcode_generator', 99 );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && gp_ACTIVE_SHORTCODE ) {
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary gp-shortcode" data-editor-id="'. $editor_id .'">'. __( 'Add Shortcode', 'gp-framework' ) .'</a>';
    }
  ...
  ?>
    <div id="gp-shortcode-dialog" class="gp-dialog" title="<?php _e( 'Add Shortcode', 'gp-framework' ); ?>">
      <div class="gp-dialog-header">
        <select class="chosen <?php echo ( is_rtl() ) ? 'chosen-rtl' : ''; ?> gp-dialog-select" data-placeholder="<?php _e( 'Select a shortcode', 'gp-framework' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="gp-dialog-load"></div>
      <div class="gp-insert-button hidden">
        <a href="#" class="button button-primary gp-dialog-insert"><?php _e( 'Insert Shortcode', 'gp-framework' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    if( isset( $shortcode['fields'] ) ) {
      ...
      foreach ( $shortcode['fields'] as $key => $field ) {
        ...
        echo gp_add_element( $field, $field_default, 'shortcode' );
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="gp-shortcode-clone" data-clone-id="'. $clone_id .'">';
      echo '<a href="#" class="gp-remove-clone"><i class="fa fa-trash"></i></a>';
      ...
      echo '<div class="gp-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';
